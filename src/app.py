...
def webhook():
        if request.method == 'POST':
            ...
            expect_sign = hmac.new(key=key, msg=request.data, digestmod=hashlib.sha1).hexdigest()
            incom_sign = request.headers.get('X-Hub-Signature').split('sha1=')[-1].strip()
            if hmac.compare_digest(incom_sign, expect_sign):
                verify_sign = True 
            if verify_sign:
